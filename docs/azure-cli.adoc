...
NOTE: En caso de dudas, consulta la documentación oficial: https://learn.microsoft.com/en-us/cli/azure/install-azure-cli?view=azure-cli-latest
...
En caso de estar en un ordenador del Aula de practicas, es recomendable usar el parámetro `--use-device-code` para evitar problemas con el navegador:
...
Abre automáticamente un navegador con la dirección https://microsoft.com/devicelogin y pega el código. Esto te permite loguearte con tus credenciales. 

NOTE: En caso de problemas de login, consulta la documentación oficial: https://learn.microsoft.com/en-us/cli/azure/authenticate-azure-cli-interactively?view=azure-cli-latest
...
A continuación, vamos a listar solo las regiones que *realmente* nos permiten desplegar VMs, es decir, las regiones que no están restringidas por *Azure Policy* u otras restricciones. Los comandos están preparados para ser lanzados en *PowerShell*: 
...
Si estás en *Windows* con Git Bash, el comando es:
...
[source,bash]
----
$ az policy assignment list --query "[?parameters.listOfAllowedLocations].parameters.listOfAllowedLocations.value[]"   -o tsv
...
Listar todos los discos en un grupo de recursos. Sustituye `MyResourceGroupNAME` por el nombre de tu grupo de recursos.

[source,bash]
----
az disk list -o table -g MyResourceGroupNAME
----
...
[source,bash]
----
az disk list -g MyResourceGroupNAME --query "[?sku.name=='StandardSSD_LRS']" -o table
----
...
*Standard HDD OS disks* will be retired on September 8, 2028. For new OS disks, use Standard SSD or Premium SSD instead. Migrate existing Standard HDD OS disks to Standard SSD or Premium SSD. https://go.microsoft.com/fwlink/?LinkId=2338468[Click to learn more]
